...
- (void) load:(LoadAdRequest*) loadAdRequest {
	...
	[GADRewardedInterstitialAd loadWithAdUnitID:loadAdRequest.adUnitId request:gadRequest completionHandler:^(GADRewardedInterstitialAd* ad, NSError* error) {
		if (error) {
			NSLog(@"Failed to load RewardedInterstitialAd with error: %@", [error localizedDescription]);
			...
			NSLog(@"RewardedInterstitialAd %@ loaded successfully", self.adId);
...
- (void) show {
	if (self.gadAd) {
		...
		NSLog(@"RewardedInterstitialAd show: ad not set");
	}
}
...
- (void) adWillPresentFullScreenContent:(nonnull id<GADFullScreenPresentingAd>) ad {
	...
	if (AdFormatBase.pauseOnBackground) {
		NSLog(@"RewardedInterstitialAd pauseOnBackground");
