...
- (void) load:(LoadAdRequest*) loadAdRequest {
	...
	[GADRewardedAd loadWithAdUnitID:loadAdRequest.adUnitId request:gadRequest completionHandler:^(GADRewardedAd* ad, NSError* error) {
		if (error) {
			NSLog(@"failed to load RewardedAd with error: %@", [error localizedDescription]);
			...
			NSLog(@"RewardedAd %@ loaded successfully", self.adId);
...
- (void) show {
	if (self.gadAd) {
		...
		NSLog(@"RewardedAd show: ad not set");
	}
}
...
- (void) ad:(nonnull id<GADFullScreenPresentingAd>) ad didFailToPresentFullScreenContentWithError:(nonnull NSError *) error {
	NSLog(@"RewardedAd didFailToPresentFullScreenContentWithError");
...
- (void) adWillPresentFullScreenContent:(nonnull id<GADFullScreenPresentingAd>) ad {
	NSLog(@"RewardedAd adWillPresentFullScreenContent");
	...
	if (AdFormatBase.pauseOnBackground) {
		NSLog(@"RewardedAd pauseOnBackground");
		OS_IOS::get_singleton()->on_focus_out();
	}
}

- (void) adDidDismissFullScreenContent:(nonnull id<GADFullScreenPresentingAd>) ad {
	NSLog(@"RewardedAd adDidDismissFullScreenContent");
